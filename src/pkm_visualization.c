...
void pkm_draw_background(cairo_t *context,
						unsigned int width,
						unsigned int height,
						double color_r,
						double color_g,
						double color_b)
{
	cairo_set_source_rgb(context, color_r, color_b, color_g);
...
void pkm_draw_point(cairo_t *context,
...
void pkm_visualize_kmeans(pkm_centroid **centrs, size_t num_centrs, unsigned int dim1, unsigned int dim2)
{
	...
	pkm_draw_background(cr, width, height, 1.0, 1.0, 1.0);
